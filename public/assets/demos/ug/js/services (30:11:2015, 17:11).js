...
function makeSparkles(){
	...
	particleCount = 0,
	  gravity = 1,
	  colors = [
	  	'#000', '#FFF', '#CCC', '#92724f','#FFD700'/*, '#D7D6C9'*/
	  ];
	particle2 = [];
	particle2Count = 0;
	...
	for (var i = 0; i < 600; i++) {
	  particle2.push({
	    ...
	    draw: function() {
	      ...
	      context.beginPath();
	      var randSq = Math.round(Math.random()*7);
...
//jQuery slider control   
function sliderControlMob(){
	$('.pullerCont').children('#grab').remove();
	$('.pullerMessage').removeClass('pullerMessage').addClass('pressToPull');
	//$('.pullerCont').css('background-image','url(images/slider-track-btn.png)');
	$('.pressToPull').click(function(){
		crack();
	});
};
...
$(document).ready(function(){
	...
	/*var botSparkles = [];
	for (var i = 0; i < (sparkNum/3); i++) {
	  botSparkles[i] = new sparkle((Math.random()*40)+(sparkNum/10));
